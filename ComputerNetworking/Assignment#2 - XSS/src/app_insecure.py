...
def login():
    ...
    if request.method == 'POST':
        if (username, password) in [('brandon', 'atigdng'), ('sam', 'xyzzy')]:
            ...
            ## print hacker's URL on the terminal (added at login() to print just once.)
            with open('attack.js') as f:
                query={'flash':f.read().strip().replace('\n', ' ')}
            hackerMSG="THANKS" ## string for greenbox
            hackerURL='http://localhost:5000/?'+urlencode(query)+hackerMSG
            print('\nhacker\'s URL : '+hackerURL+'\n')
            ##
            return response
...
def pay():
    ...
    if request.method == 'POST':
        if account and dollars and dollars.isdigit() and memo:
            ...
            db.commit()
            if account == 'hacker': ## to check is account hacker or not
                return redirect(url_for('index', flash='Hacking successful'))
            else:
